...
- **Zero-knowledge proxy** — run any agent or script behind a local proxy that injects credentials at request time, without ever exposing them in environment variables or process args.
...
- **Multiple accounts per provider** — log into GitHub as your personal account and your work account, then pick which one each agent uses.
- **Web dashboard** — view and manage all your connections in a local browser UI.
- **Audit log** — a complete record of every credential access, login, and logout.
- **Per-agent access control** — decide which agents are allowed to use which credentials, without changing the agent itself.
- **Firewall rules** — block or allow outbound requests at the proxy layer per agent and per service.
...
- **Multi-user support** — share a single authsome instance across multiple users, with proper credential isolation between them.

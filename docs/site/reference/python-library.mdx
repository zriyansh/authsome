...
`create_auth_service()` is the same constructor the daemon uses. It reads `~/.authsome/config.json`, opens the active profile's vault, wires the provider registry, and returns an `AuthService`. Use it when you want one-line library access against the standard `~/.authsome/` install.
...
Equivalent to `uvx authsome register ./acmecrm.json`. The schema is documented in [Provider schema](/reference/provider-schema).

...
`authsome doctor` verifies the configured backend is reachable, and `authsome whoami` shows both the configured mode and the effective source chosen at runtime.

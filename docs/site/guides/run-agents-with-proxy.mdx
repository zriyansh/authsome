...
The proxy is HTTP(S)-only and uses each provider's default connection. The full routing contract and the list of known limitations is on [Proxy injection](/concepts/proxy-injection).

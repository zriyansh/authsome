...
<Update label="0.2.4" description="2026-05-08">
  **Interactive dashboard.** The local daemon now serves an interactive dashboard at `/overview`. Lime-themed UI matching the marketing site palette. Per-provider detail panes for OAuth and API-key connections.
...
<Update label="0.2.0" description="2026-04-28">
  **Architectural restructure.** Vault and AuthLayer split into separate layers with a documented protocol boundary. Public Python API moved (`AuthClient` → `AuthsomeContext`). CLI commands and flags are unchanged. See [Architecture](/concepts/architecture).
  ...
  **mitmproxy CA injection.** Subprocesses started by `authsome run` get a combined system + mitmproxy CA bundle so SDKs trust the proxy by default.
  ...
  ⚠ **Breaking:** `AuthClient` is replaced by `AuthsomeContext`. Library users should switch to `from authsome.context import AuthsomeContext`. See [Python library](/reference/python-library).
</Update>

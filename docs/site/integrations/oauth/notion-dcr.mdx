...
The first time, authsome opens a local form at `http://127.0.0.1:7999` to collect your `client_id` and `client_secret`. They are encrypted under your profile and reused on every subsequent login. A second browser window then opens to `https://mcp.notion.com/authorize` for the authorization step.
...
token = ctx.auth.get_access_token("notion_dcr", connection="default")
```

<ProxyInjection envVar="NOTION_ACCESS_TOKEN" secretLabel="token" host="mcp.notion.com" />

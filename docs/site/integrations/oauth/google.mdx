...
The first time, authsome opens a local form at `http://127.0.0.1:7999` to collect your `client_id` and `client_secret`. They are encrypted under your profile and reused on every subsequent login. A second browser window then opens to `https://accounts.google.com/o/oauth2/v2/auth` for the authorization step.
...
token = ctx.auth.get_access_token("google", connection="default")
```

<ProxyInjection envVar="GOOGLE_ACCESS_TOKEN" secretLabel="token" host="regex:.*googleapis.*" />

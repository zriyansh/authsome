---
title: "Buffer"
sidebarTitle: "Buffer"
description: "Store and use your Buffer API key with authsome. Local encrypted vault, proxy injection, no key in the agent's environment."
icon: "buffer"
keywords: ["buffer api key authsome", "buffer key broker", "buffer agent credentials", "authsome buffer"]
---

import ProxyInjection from "/snippets/proxy-injection.mdx";
import MultiConnectionsCta from "/snippets/multi-connections-cta.mdx";
import MaskedInputNote from "/snippets/masked-input-note.mdx";
import WhatsNextApiKey from "/snippets/whats-next-apikey.mdx";

Buffer uses a long-lived API key. Authsome stores the key encrypted in the local vault, injects it at request time through the proxy, and keeps it out of shell history, process listings, and environment dumps.

## At a glance

| | |
|---|---|
| Provider name | `buffer` |
| Display name | Buffer |
| Auth type | API key |
| Header | `Authorization: Bearer <key>` |
| Proxy host | `api.bufferapp.com` |
| Env var | `BUFFER_API_KEY` |
| Where to get a key | [https://publish.buffer.com/account/apps](https://publish.buffer.com/account/apps) |

## Get a key

Create an API key in the Buffer dashboard at [https://publish.buffer.com/account/apps](https://publish.buffer.com/account/apps).

## Log in

```bash
uvx authsome login buffer
```

A local browser form opens at `http://127.0.0.1:7999`. Paste the key into the masked input and submit.

<MaskedInputNote />

Verify:

```bash
uvx authsome get buffer --field status
# → connected
```

## Use the key

<ProxyInjection envVar="BUFFER_API_KEY" secretLabel="key" host="api.bufferapp.com" />

### From a Python agent

```python
from authsome.context import AuthsomeContext

ctx = AuthsomeContext.create()
key = ctx.auth.get_access_token("buffer", connection="default")
```

### As an environment variable

```bash
eval "$(uvx authsome export buffer --format env)"
echo $BUFFER_API_KEY
```

## Multiple keys

<MultiConnectionsCta />

```bash
uvx authsome login buffer --connection personal
uvx authsome login buffer --connection team
```

## Rotate or remove the key

```bash
uvx authsome login buffer --force          # rotate
uvx authsome logout buffer                  # remove local credential
uvx authsome remove buffer                  # remove all local state for the provider
```

API-key providers have no revocation endpoint, so `revoke` and `remove` are equivalent for Buffer.

## Override the bundled definition

```bash
uvx authsome inspect buffer > ~/.authsome/providers/buffer.json
# edit fields
uvx authsome list   # source now shows "custom" for buffer
```

See [Provider schema](/reference/provider-schema) for every field.

## What's next

<WhatsNextApiKey />

...
A local browser form opens at `http://127.0.0.1:7999`. Paste the key into the masked input and submit. The terminal prints `Successfully logged in to openai (default).`
...
<ProxyInjection envVar="OPENAI_API_KEY" secretLabel="key" host="api.openai.com" />

If you use the official OpenAI Python SDK, no code change is required: the SDK reads `OPENAI_API_KEY` from the environment, makes the request, and the proxy substitutes the header on the way out.
...
Use this when a tool can't route through an HTTP proxy or when you need the key in a long-lived shell.

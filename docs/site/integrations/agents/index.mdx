...
The supported pattern for every framework is the same: **wrap the agent invocation with `uvx authsome run --`**. The local HTTP proxy intercepts outbound requests and injects the right `Authorization` header per provider. The agent's environment never contains real secrets.

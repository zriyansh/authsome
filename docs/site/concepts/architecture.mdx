...
- **Library mode**. `from authsome.server.dependencies import create_auth_service`. A direct programmatic API.

<Frame caption="Sidecar-mode call graph. Identity and policy gate the vault; the auth layer refreshes when needed; the sidecar writes back, injects the Authorization header, and audits at every step.">
```text
agent

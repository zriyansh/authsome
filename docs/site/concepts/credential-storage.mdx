...
`master.key` has mode `0600`. The user-registered provider files override bundled definitions of the same name.
...
Writes acquire an advisory <Tooltip tip="POSIX file lock, cooperative. Processes that respect the lock serialize; processes that ignore it can still write. Authsome respects it.">`fcntl.flock`</Tooltip> on the per-profile `lock` file. Multiple authsome processes can read concurrently but serialize writes within the same profile. Locks across profiles are independent.

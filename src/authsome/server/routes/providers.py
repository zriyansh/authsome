...
@router.get("")
async def list_providers(auth: AuthService = Depends(get_auth_service)):
...
@router.get("/{provider}")
async def get_provider(provider: str, auth: AuthService = Depends(get_auth_service)):
    return (await auth.get_provider(provider)).model_dump(mode="json")


@router.post("")
async def register_provider(body: dict, auth: AuthService = Depends(get_auth_service)):
...
async def delete_provider(provider: str, auth: AuthService = Depends(get_auth_service)):
